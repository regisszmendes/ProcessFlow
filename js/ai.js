      ...
      outputDiv.innerHTML = `
        <div class="ai-output">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;padding-bottom:1rem;border-bottom:2px solid rgba(124,58,237,.2);">
            ...
            <button onclick="saveGeneratedPlan()" style="padding:10px 20px;background:#7c3aed;color:white;border:none;border-radius:6px;cursor:pointer;font-weight:700;font-size:14px;">💾 Save This Plan</button>
          </div>
...
window.saveGeneratedPlan = function() {
  ...
  const proc = window.processes.find(p => p.id === window.currentAIPlan.process_id);
  const title = prompt('Give this plan a name:', `${proc?.name || 'Process'} - Plan ${window.savedAIPlans.length + 1}`);
...
// RENDER STAGED PLANS
window.renderStagedPlans = function() {
  const container = document.getElementById('staged-plans-container');
  
  if (!container) return;
  
  if (window.savedAIPlans.length === 0) {
    container.innerHTML = '<div style="text-align:center;padding:2rem;color:#999;">No saved plans yet. Generate and save plans above.</div>';
    return;
  }

  const html = window.savedAIPlans.map((plan, index) => {
    const proc = window.processes.find(p => p.id === plan.process_id);
    const procName = proc ? proc.name : 'Unknown Process';
    
    return `
      <div class="card" style="margin-bottom:1rem;">
        <div class="card-header" style="cursor:pointer;" onclick="toggleStagedPlan('${plan.id}')">
        ...
        <div id="staged-plan-${plan.id}" style="display:none;padding:1.5rem;border-top:1px solid #e5e5e5;">
          <div style="white-space:pre-wrap;line-height:1.8;font-size:14px;max-height:400px;overflow-y:auto;">${formatAIOutput(plan.content)}</div>
          <div style="margin-top:1rem;padding-top:1rem;border-top:1px solid #e5e5e5;font-size:12px;color:#666;">
            Generated by ${plan.model} • ${new Date(plan.generated_at).toLocaleString()}
          </div>
        </div>
      </div>
    `;
...
// TOGGLE STAGED PLAN VISIBILITY
window.toggleStagedPlan = function(planId) {
  const details = document.getElementById(`staged-plan-${planId}`);
  if (details) {
    details.style.display = details.style.display === 'none' ? 'block' : 'none';
  }
};

// SEND PLAN TO MONITORING
window.sendPlanToMonitoring = async function(planId) {
  ...
  if (!confirm(`Send "${plan.title}" to Monitoring?\n\nYou'll be able to track KPIs and progress there.`)) {
    return;
  }

  const sections = parseAIPlan(plan.content);
...
function parseAIPlan(content) {
  ...
  lines.forEach(line => {
    if (line.match(/^##\s*(EXECUTIVE SUMMARY|Executive Summary)/i)) {
      ...
      currentContent = [];
    } else if (line.match(/^##\s*(KEY ISSUES|Key Issues)/i)) {
      if (currentSection) sections[currentSection] = currentContent.join('\n').trim();
      ...
      currentContent = [];
    } else if (line.match(/^##\s*(MEDIUM-TERM|Medium-term)/i)) {
      if (currentSection) sections[currentSection] = currentContent.join('\n').trim();
...
console.log('✅ AI with staging area fully loaded');
